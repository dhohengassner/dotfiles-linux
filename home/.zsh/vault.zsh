...
function vt() {
	...
	awsaccounts=(lab1 lab2 hyperloop prd prd_read appsjw brdcst prd_leg travel_dev travel_prd)
	...
	select opt in "${awsaccounts[@]}"; do
		...
		hyperloop)
			...
			;;
		prd)
			vaulty_authy "$(echo $bethel_values | jq -r '.vault.environment_creds.prd')" "$1" $bethel_values
			break
			;;
		prd_read)
			vaulty_authy "$(echo $bethel_values | jq -r '.vault.environment_creds.prd_read')" "$1" $bethel_values
			break
			;;
		appsjw)
		...
		brdcst)
			...
			;;
		prd_leg)
		  vaulty_authy "$(echo $bethel_values | jq -r '.vault.environment_creds.prd_leg')" "$1" $bethel_values
			break
			;;
		travel_dev)
		  vaulty_authy "$(echo $bethel_values | jq -r '.vault.environment_creds.travel_dev')" "$1" $bethel_values
			break
			;;
		travel_prd)
		  vaulty_authy "$(echo $bethel_values | jq -r '.vault.environment_creds.travel_prd')" "$1" $bethel_values
			break
			;;
		*)
